...
function getSDLRequestGroupsByUserId(userId, sdlResourceName) {
  ...
  if (!cache.dataByUserId[userId][sdlResourceName]) {
    ...
    const sdlDataRequestGroupsText = fs.readFileSync(path.join(__dirname, 'scenario', `${sdlResourceName}.json`), { encoding: 'utf-8' });
...
export async function step1(userId) {
  await sdlHttpRequestsRunner.run(sdlServerOptions, getSDLRequestGroupsByUserId(userId, 'step01-sdl'));
}
export async function step2(userId) {
  await sdlHttpRequestsRunner.run(sdlServerOptions, getSDLRequestGroupsByUserId(userId, 'step02-sdl'));
}
