...
export async function nextStep(userId) {
  ...
  const sdlResource = `step0${stage}-sdl`
  const neoneResource = `step0${stage}-neone`
  if (stage === 1 || stage === 2) {
    await sdlHttpRequestsRunner.run(sdlServerOptions(), getSDLRequestGroupsByUserId(userId, sdlResource));
  } else if (stage >= 4) {
    const requestGroups = getSDLRequestGroupsByUserId(userId, sdlResource);
  ...
  if(stage >= 3 && stage <= 5) {
    await neoneHttpRequestsRunner.run(neoneServerOptions(), getNEONERequestGroupsByUserId(userId, neoneResource));
  }
  ...
  if (stage !== 3) {
    ...
    const newRequests = cache.dataByUserId[userId][sdlResource][0].requests
  ...
  if (stage >= 3 && stage <= 5) {
    const currentRequests = cache.dataByUserId[userId].currentStage[0]?.requests
    const newRequests = cache.dataByUserId[userId][neoneResource][0].requests
