...
function getSDLRequestGroupsByUserId(userId, sdlResourceName) {
  ...
  if (!cache.dataByUserId[userId][sdlResourceName]) {
    ...
    const sdlDataRequestGroups = JSON.parse(
      sdlDataRequestGroupsText
        .replaceAll(/"(\{geodis\}\/tenants\/[^"]*)"/gm, `"$1_${userId}"`)
        .replaceAll(/"(https:\/\/neone.onerecord.fr\/logistics-objects\/[^"]*)"/gm, `"$1_${userId}"`)
    );
...
function getNEONERequestGroupsByUserId(userId, neoneResourceName) {
  ...
  if (!cache.dataByUserId[userId][neoneResourceName]) {
    const neoneDataRequestGroupsText = fs.readFileSync(path.join(__dirname, 'scenario', `${neoneResourceName}.json`), { encoding: 'utf-8' });

    // suffix all links by userId and parse
    /** @type {import('http-requests-runner').RequestGroup[]} */
    const neoneDataRequestGroups = JSON.parse(
      neoneDataRequestGroupsText
        .replaceAll(/"(\{geodis\}\/tenants\/[^"]*)"/gm, `"$1_${userId}"`)
        .replaceAll(/"(https:\/\/neone.onerecord.fr\/logistics-objects\/[^"]*)"/gm, `"$1_${userId}"`)
    );
...
export async function nextStep(userId) {
  ...
  console.log("Currently executing step", stage, "for user", userId)

  switch (stage) {
    case 3:
      await neoneHttpRequestsRunner.run(neoneServerOptions(), getNEONERequestGroupsByUserId(userId, `step0${stage}-neone`));
      pushNEOneValues(userId, stage)
      break;
    case 4:
      await stage4(userId)
      break;
    default:
      await sdlHttpRequestsRunner.run(sdlServerOptions(), getSDLRequestGroupsByUserId(userId, `step0${stage}-sdl`));
      pushSDLValues(userId, stage)
      break;
  }

  // add stage value to currentStage...
  cache.dataByUserId[userId].currentStage[0].stage = stage;

  return cache.dataByUserId[userId].currentStage
}

function pushNEOneValues(userId, stage) {
  const resource = `step0${stage}-neone`
  ...
  currentRequests.push(...newRequests.map(req => {
    return {
      ...req,
      "type": req.body["@type"],
      "params": {
...
function pushSDLValues(userId, stage) {
  ...
  for (const newRequest of newRequests) {
    if (newRequest.action === "update_lo") {
      ...
      const toBeUpdated = currentRequests.find(req => req.action === "create_lo" && req?.params?.id === id && req.type === newRequest.type)
      if (toBeUpdated) {
        toBeUpdated.body = newRequest.body
      }
    } else if (newRequest.action === "create_event") {
      // Ignore events
    }
    else {
...
async function stage4(userId) {
  const requestGroups = getSDLRequestGroupsByUserId(userId, `step04-sdl`);
  ...
  const eventsByLO = requestGroups[0].requests.filter(r => r.action === 'create_event').reduce((acc, request) => {
    ...
    acc[request.loId].events.push({
      ...
      uri: `{geodis}/tenants/geodis/${sdlService.typeToResourceType[request.type]}/${request.loId}_${userId}/events/${request.params.id}`
    });
  ...
  await neoneHttpRequestsRunner.run(neoneServerOptions(), getNEONERequestGroupsByUserId(userId, 'step04-neone'));
}
...
export async function reset(userId) {
  ...
  try {
    ...
    await neoneHttpRequestsRunner.run(neoneServerOptions(), getNEONERequestGroupsByUserId(userId, 'delete-neone'));
  } catch (error) {
    // FIXME ignored
    console.log(error);
  }
