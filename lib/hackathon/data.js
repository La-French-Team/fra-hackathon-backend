...
export async function nextStep(userId) {
  ...
  if (stage === 1 || stage === 2) {
    ...
    await neoneHttpRequestsRunner.run(neoneServerOptions(), getNEONERequestGroupsByUserId(userId, 'step03-neone'));
  } else if (stage === 4) {
  ...
  if (stage !== 3) {
    ...
    const currentRequests = cache.dataByUserId[userId].currentStage[0]?.requests
    const newRequests = cache.dataByUserId[userId]["step03-neone"][0].requests
